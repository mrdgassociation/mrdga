...
import { 
  ...
  X, ArrowLeft, ArrowRight, Radio, Info, Loader2, PlayCircle, Award, FileText, Users, Clock, Trash, Hash, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export default function Competitions() {
  ...
  const generateDynamicCompId = (selectedDateStr, existingCompetitions = []) => {
    ...
    return `COMP-${targetYear}-${nextSerial}`;
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
              <div className="lg:col-span-7 space-y-4">
                {getEmbedYoutubeUrl(activeDetailComp.youtubeUrl) ? (
                  ...
                  /* 🖼️ मल्टीपल इमेजेस आणि ऑटो-स्क्रोल सह फोटो कॅरोसेल */
                  <ImageCarousel 
                    images={[
                      activeDetailComp.bannerUrl, 
                      ...(Array.isArray(activeDetailComp.galleryImages) ? activeDetailComp.galleryImages : [])
                    ].filter(Boolean)} 
                  />
                )}
...
// 🖼️ 🎯 UPDATED AUTO SCROLL IMAGE CAROUSEL COMPONENT
function ImageCarousel({ images = [] }) {
  const [currentIndex, setCurrentIndex] = useState(0);

  // 🔄 ३.५ सेकंदांनंतर ऑटो स्क्रोल होण्यासाठीचा इफेक्ट
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-[320px] sm:h-[380px] rounded-2xl overflow-hidden border border-amber-500/20 bg-black/90 flex items-center justify-center group shadow-xl">
      
      {/* मुख्य इमेज */}
      <img 
        key={currentIndex}
        src={images[currentIndex]} 
        alt={`Competition Photo ${currentIndex + 1}`} 
        className="w-full h-full object-contain transition-all duration-500 ease-in-out"
        onError={(e) => { e.target.src = './event-banner.jpg'; }}
      />

      {/* डावा व उजवा नेव्हिगेशन बाण (Left/Right Arrows) */}
      {images.length > 1 && (
        <>
          <button 
            onClick={handlePrev}
            className="absolute left-2.5 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/70 text-white border border-white/20 hover:bg-amber-500 hover:text-black transition cursor-pointer opacity-80 sm:opacity-0 group-hover:opacity-100"
            title="मागील फोटो"
          >
          ...
          <button 
            onClick={handleNext}
            className="absolute right-2.5 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/70 text-white border border-white/20 hover:bg-amber-500 hover:text-black transition cursor-pointer opacity-80 sm:opacity-0 group-hover:opacity-100"
            title="पुढील फोटो"
          >
            <ChevronRight className="w-5 h-5" />
          </button>

          {/* खालील डॉट्स इंडिकेटर्स (Dots Indicator) */}
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 bg-black/70 px-3 py-1.5 rounded-full border border-white/10 backdrop-blur-sm">
            {images.map((_, idx) => (
              <button
                key={idx}
                onClick={(e) => {
